...
type Upgrade = { version: number; upgradeSql: string; fileName: string };
...
export function registerDbInitialisedListener(listener: () => void) {
	if (initialised) {
		listener();
	} else {
...
async function getUpgrades(startingFromVersion: number) {
