...
export function initMaintenanceJobs(client: Client) {
...
function addEnsureGuildCommandsUpdatedJob(client: Client) {
	const rule = new schedule.RecurrenceRule();
	rule.hour = 6;
	rule.minute = 15;
	rule.tz = 'Etc/UTC';
...
function addEnsureWebhookCorrectnessJob(client: Client) {
	const rule = new schedule.RecurrenceRule();
	rule.hour = 6;
	rule.minute = 30;
	rule.tz = 'Etc/UTC';
